...
export const executeCode = async (req, res) => {
  try {
    ...
    let allPassed = true; //For all test cases passed or not
    ...
    const detailedResults = results.map((result, i) => {
      ...
      return {
        ...
        complie_output: result.compile_output || null,
    ...
    const submission = await db.submission.create({
      data: {
        ...
        stdin: stdin.join("\n"),
        stdout: JSON.stringify(detailedResults.map((r) => r.stdout)), //Stringify for storing in db
    ...
    // If all passed then mark problem as solved for the current user

    if (allPassed) {
      // upsert - If record does not exist create it otherwise update it
      await db.problemSolved.upsert({
        ...
        update: {}, //If entry already exists we are not doing anything
        create: {
          userId,
          problemId,
        },
